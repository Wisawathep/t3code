...
const handlers = {
  ...
  preview_recording_stop: (input) =>
    Effect.gen(function* () {
      ...
      const provider = McpInvocationContext.getProviderSessionPrincipal(scope);
      if (!provider) return yield* Effect.die("Preview recording requires a provider session.");
      return yield* claimPreviewRecording(provider.threadId, response);
    }),
